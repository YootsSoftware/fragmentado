...
const CONFIG_TABS = [
  { id: 'site', label: 'Sitio público', icon: faGlobe },
  { id: 'spotify', label: 'Spotify', icon: faMusic },
  { id: 'account', label: 'Cuenta', icon: faUserShield },
];

function ConfigCardHead({ icon, title, description }) {
  return (
  ...
  return (
    <>
      ...
      <p className={styles.editorHint}>
        Configura el sitio, las integraciones y el acceso al panel.
      </p>

      <div className={styles.configTabs} role="tablist" aria-label="Categorías de ajustes">
        {CONFIG_TABS.map((tab) => (
          <button
            key={tab.id}
            type="button"
            id={`config-tab-${tab.id}`}
            className={activeTab === tab.id ? styles.configTabActive : styles.configTab}
            role="tab"
      ...
      <div className={styles.configWrap}>
        {activeTab === 'site' ? (
          <section
            ...
            id="config-panel-site"
            role="tabpanel"
            aria-labelledby="config-tab-site"
          >
            <ConfigCardHead
              icon={faGear}
              title="Sitio público"
              description="Identidad, hero y redes sociales de Fragmentado."
            />
            <form className={`${styles.form} ${styles.configForm}`} onSubmit={onSaveSettings}>
              <div className={styles.configGridTwo}>
                <label className={styles.configFieldSpanTwo}>
                  Nombre artístico global
                  <input
                ...
                <div className={`${styles.configFieldSpanTwo} ${styles.configGroupHeading}`}>
                  <strong>Hero principal</strong>
                  <p>Selecciona el contenido que recibe a los visitantes.</p>
                </div>
                <label>
                  Tipo de fondo
                  <select
                ...
                {settingsDraft.hero?.mediaType !== 'image' ? (
                  ...
                  <div className={styles.configStaticMediaNote}>
                    Se usará la imagen actual de Pausa al Amor.
                  </div>
                )}
                <p className={`${styles.inlineNote} ${styles.configFieldSpanTwo}`}>
                  El video se reproduce silenciado y en bucle. El hero permanece negro hasta que esté listo.
                </p>

                <div className={`${styles.configFieldSpanTwo} ${styles.configGroupHeading}`}>
                  <strong>Redes sociales</strong>
                  <p>Estos enlaces aparecen en el hero y en contrataciones.</p>
                </div>
                <label>
                  Facebook
                  <input
                    ...
                    onChange={(event) =>
                      setSettingsDraft((prev) => ({
                        ...prev,
                        socials: { ...(prev.socials ?? {}), facebook: event.target.value },
                      }))
                    }
                  />
                </label>
              ...
              <div className={`${styles.actions} ${styles.configActions}`}>
                <button type="submit" className={styles.buttonSuccess}>Guardar sitio</button>
              </div>
            </form>
          </section>
        ) : null}
        ...
        {activeTab === 'spotify' ? (
          <section
            className={`${styles.platformsSection} ${styles.configCard}`}
            id="config-panel-spotify"
            role="tabpanel"
            aria-labelledby="config-tab-spotify"
          >
            <ConfigCardHead
              icon={faMusic}
              title="Integración con Spotify"
              description="Consulta el catálogo del artista e importa canciones a los discos."
            />

            <dl className={styles.configStatusList}>
              <div>
                <dt>Conexión</dt>
                <dd>{spotifyEnv.configured ? 'Configurada' : 'Incompleta'}</dd>
              </div>
              <div>
                <dt>Artist ID</dt>
                <dd>{spotifyEnv.artistId || 'Sin configurar'}</dd>
              </div>
              <div>
                <dt>Mercado</dt>
                <dd>{spotifyEnv.market || 'MX'}</dd>
              </div>
            </dl>
            <p className={styles.inlineNote}>
              La conexión se administra con <code>SPOTIFY_CLIENT_ID</code>,{' '}
              <code>SPOTIFY_CLIENT_SECRET</code>, <code>SPOTIFY_ARTIST_ID</code> y{' '}
              <code>SPOTIFY_MARKET</code> en el servidor.
            </p>
            <div className={`${styles.actions} ${styles.configActions}`}>
              <button
                type="button"
            ...
            {spotifySongs.length ? (
              <div className={styles.statsBox}>
                <p>
                  Álbumes: <strong>{spotifyMeta.totalAlbums}</strong> | Canciones:{' '}
        ...
        {activeTab === 'account' ? (
          <section
            className={`${styles.platformsSection} ${styles.configCard} ${styles.configAccountCard}`}
          ...
          >
            <ConfigCardHead
              ...
              description="Actualiza el usuario y la contraseña de acceso a fg-admin."
            />
